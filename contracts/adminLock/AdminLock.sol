...
contract AdminLock is Ownable {
    uint256 public constant LOCK_PERIOD = 8467200;  //14 week
    ...
    function withdrawLP(uint256 pid) public {
        require(
            block.timestamp >= _startTimestamp[msg.sender][pid] + LOCK_PERIOD,
            "STILL LOCKED."
        );

        uint256 amount = _lp.balanceOf(address(this));
